		...
		if os.path.exists(video_file):
			...
			for animation_idx in range(len(input_list)):
				...
				for step in range(input.shape[0]):
					...
					cv2.line(image_new, (frame_width - 270, 30), (frame_width - 250, 30), (0, 0, 0), 4)
					cv2.putText(image_new, "Real Trajectory", (frame_width - 240, 30), font, 0.5, (0, 0, 0), 2, cv2.LINE_AA)
					cv2.line(image_new, (frame_width - 270, 50), (frame_width - 250, 50), (0, 0, 255), 4)
					if self.args.output_pred_state_dim == 5:
						cv2.putText(image_new, "Predicted Trajectory Prob " + str(np.sum(pis[:,0])/self.args.prediction_horizon*100), (frame_width - 240, 50), font, 0.5, (0, 0, 255), 2,
						            cv2.LINE_AA)
						cv2.line(image_new, (frame_width - 270, 70), (frame_width - 250, 70), (0, 255, 0), 4)
						cv2.putText(image_new, "Predicted Trajectory Prob " + str(np.sum(pis[:,1])/self.args.prediction_horizon*100), (frame_width - 240, 70), font, 0.5, (0, 255, 0), 2,
						            cv2.LINE_AA)
						cv2.line(image_new, (frame_width - 270, 90), (frame_width - 250, 90), (0, 255, 255), 4)
						cv2.putText(image_new, "Predicted Trajectory Prob " + str(np.sum(pis[:,2])/self.args.prediction_horizon*100), (frame_width - 240, 90), font, 0.5, (0, 255, 255), 2,
						            cv2.LINE_AA)
						cv2.line(image_new, (frame_width - 270, 110), (frame_width - 250, 110), (255, 153, 51), 4)
					else:
						cv2.putText(image_new, "Predicted Trajectory: " + str(np.around(traj_likelihood[step][0,0,0],decimals=2)), (frame_width - 240, 50), font, 0.5, (0, 0, 255), 2,
						            cv2.LINE_AA)
						cv2.line(image_new, (frame_width - 270, 70), (frame_width - 250, 70), (0, 255, 0), 4)
						cv2.putText(image_new, "Predicted Trajectory: " + str(np.around(traj_likelihood[step][0,0,1],decimals=2)), (frame_width - 240, 70), font, 0.5, (0, 255, 0), 2,
						            cv2.LINE_AA)
						cv2.line(image_new, (frame_width - 270, 90), (frame_width - 250, 90), (0, 255, 255), 4)
						cv2.putText(image_new, "Predicted Trajectory: " + str(np.around(traj_likelihood[step][0,0,2],decimals=2)), (frame_width - 240, 90), font, 0.5, (0, 255, 255), 2,
						            cv2.LINE_AA)
						cv2.line(image_new, (frame_width - 270, 110), (frame_width - 250, 110), (255, 153, 51), 4)
					cv2.putText(image_new, "1-sigma Uncertainty", (frame_width - 240, 110), font, 0.5, (255,153,51), 2,
					            cv2.LINE_AA)
