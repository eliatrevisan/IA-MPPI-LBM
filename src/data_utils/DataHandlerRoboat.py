...
class DataHandlerRoboat():
	...
	def processData(self, **kwargs):
		...
		data_pickle = self.args.data_path + self.args.scenario + "/data" + str(self.args.prediction_horizon) + "_" + str(
			...
			self.args.prev_horizon) + "_roboat.pickle"
		#data_pickle = self.args.data_path + self.args.scenario + "/data" + str(self.args.prediction_horizon) + "_" + str(
		#	self.args.truncated_backprop_length)+ "_" + str(
		...
		if os.path.isfile(data_pickle):
			...
			if "roboat" in data_pickle:
				print("Processing roboat data.")
				self._process_roboat_data_(**kwargs)
			else:
				print("No roboat data present")
	...
	def load_map(self,**kwargs):
		...
		with open(os.path.join(self.data_path+self.args.scenario, 'map.json')) as json_file:
			map_data = json.load(json_file)

		grid_map = pl.imread(self.data_path+self.args.scenario + '/map.png')
		
		if self.args.scenario == 'open_crossing' or self.args.scenario == "test_open_crossing":
			grid_map = np.round(grid_map)
		else:
		...
		self.agent_container.occupancy_grid.resolution = map_data['resolution'] #map_data.item(0)['Resolution']  # map resolution in [m / cell]
		self.agent_container.occupancy_grid.map_size = map_data['map_size'] #map_data.item(0)['Size']  # map size in [m]
		self.agent_container.occupancy_grid.center =  map_data['map_center'] #self.agent_container.occupancy_grid.map_size / 2.0
	...
	def _process_roboat_data_(self, **kwargs):
		"""
		...
		for sample_idx in range(pedestrian_data.shape[0]):
			if pedestrian_data[sample_idx, 0] != -1:
				...
				timestamp = np.round(pedestrian_data[sample_idx, 1],1)# + pedestrian_data[sample_idx, 2] * 1e-9  # time in seconds28.35
				pose = np.zeros([1,3])
				vel = np.zeros([1,3])
	...
	def getGroupOfTrajectoriesAsBatch(self, trajectory_idx, max_sequence_length=1000, n_trajs=1):
		...
		grid = self.agent_container.occupancy_grid.getSubmapByCoords(traj.pose_vec[0,0] * self.norm_const_x,
																																 ...
																																 self.submap_size, self.submap_size)
		sequence_length = min(max_sequence_length, traj.pose_vec.shape[0] - self.output_sequence_length-self.prev_horizon)
		batch_x = np.zeros([1, sequence_length, (self.prev_horizon+1)*self.input_dim])
	...
	def getAgentDataAtTimestep(self, query_time):
		...
		for id in self.agent_container.agent_data.keys():
			...
			if traj_for_time != None:
				...
				grid = self.agent_container.occupancy_grid.getSubmapByCoords(pose[0], pose[1], self.submap_size, self.submap_size)
	...
	def plot_local_scenario(self,batch_grid,batch_x,batch_agent_real_traj,batch_goal,other_agents_pos,batch_agent_predicted_traj,t,seq_index):
		...
		x_lim = [-self.submap_size/2, self.submap_size/2]
		y_lim = [-self.submap_size/2, self.submap_size/2]
