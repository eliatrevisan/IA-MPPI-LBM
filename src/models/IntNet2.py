      ...
      with tf.variable_scope('encoder_model') as scope:
        ...
        with tf.variable_scope('lstm_ped',reuse=tf.AUTO_REUSE) as scope:
          ...
          for i, ped in enumerate(ped_grid_series):
            ...
            ped_series = tf.unstack(ped, axis=1)
            ped_states = []
            # iterate over truncated back propagation
            for j, ped_t in enumerate(ped_series):
              ped_input = tf.unstack(ped_t, axis=1)
              ...
              ped_states.append(ped_gru_state)
            self.other_peds_states.append(ped_states)
          a = tf.transpose(tf.concat(tf.unstack(tf.stack(self.other_peds_states),axis=0),axis=2),[1,0,2])
          ...
          self.other_peds_states = tf.squeeze(tf.stack(self.other_peds_states,axis=2),axis=0)
