...
class NetworkModel():
	...
	def __init__(self, args, is_training=True, batch_size=None):
		...
		if self.args.relative_info:
			self.input_ped_grid_placeholder = tf.placeholder(dtype=tf.float32,
			                                                 shape=[self.batch_size, self.truncated_backprop_length,
			                                                        self.n_peds, self.pedestrian_vector_dim], name='ped_grid')
		...
		# Pedestrian LSTM
		self.ped_gru_states = tf.placeholder(dtype=tf.float32,
		                                     shape=[self.batch_size, self.n_peds, self.rnn_state_ped_size],
		...
		with tf.variable_scope("model") as scope:
			...
			with tf.variable_scope('encoder_model') as scope:
				...
				with tf.variable_scope('lstm_ped', reuse=tf.AUTO_REUSE) as scope:
					...
					for i, ped in enumerate(ped_grid_series):
						...
						ped_cell = tf.contrib.rnn.GRUCell(self.rnn_state_ped_size, name='ped_basic_lstm_cell' + str(i),
						                                  trainable=True)
						ped_input = tf.unstack(ped, axis=1)
						ped_gru_output, ped_gru_state = tf.contrib.rnn.static_rnn(ped_cell, ped_input, dtype=tf.float32,
						                                                          initial_state=self.ped_gru_states[:, i, :])
					...
					self.cell_outputs_series_lstm_ped = tf.unstack(
						tf.transpose(tf.concat([outputs[0], outputs[1]], axis=2), [1, 0, 2]), axis=0)
					...
					rnn_state_size_lstm_ped = self.get_weight_variable(name='w_birnn',
					                                                   shape=[self.rnn_state_size_lstm_ped * 2,
					                                                          self.rnn_state_size_lstm_ped],
			...
			with tf.variable_scope('generation_model') as scope:
				...
				W_out = self.get_weight_variable(name='w_out',
				                                 shape=[self.fc_hidden_unit_size, self.output_dim * self.prediction_horizon],
				                                 trainable=self.is_training)
	...
	def run(self, sess, feed_dict_train):
		...
		return batch_loss, 0, _model_prediction, _summary_str, lr, 0, output_decoder, 0, 0, 0, 0
	...
	def get_diversity_loss(self, mux, muy, sigmax, sigmay):
		...
		for mix_id in range(self.args.n_mixtures):
			...
			if mix_id + 1 < self.args.n_mixtures:
				...
				sigma2 = tf.concat(
					[tf.expand_dims(sigmax[:, mix_id + 1], axis=1), tf.expand_dims(sigmay[:, mix_id + 1], axis=1)], 1)
