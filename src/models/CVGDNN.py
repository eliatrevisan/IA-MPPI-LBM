...
if sys.version_info[0] < 3:
	...
	from irnn_cell import VariationalRNNCell as vrnn_cell
	from tf_utils import *
else:
	from src.external.irnn_cell import VariationalRNNCell as vrnn_cell
...
class NetworkModel():
	...
	def __init__(self, args, is_training=True, batch_size=None):
		...
		with tf.variable_scope("model") as scope:
			...
			with tf.variable_scope('encoder_model') as scope:
				...
				concat_lstm = []
				for lstm_out, ped_feature in zip(self.cell_outputs_series_state,
				                                                     ped_grid_series):
					concat_lstm.append(tf.concat([lstm_out, ped_feature], axis=1,
					                             name="concatenate_lstm_out_with_grid_and_ped"))
			...
			with tf.variable_scope('generation_model') as scope:
				...
				dec_fc_layer2 = tf.unstack(tf.stack(tf.nn.elu([output for output in dec_fc_layer])))
				dec_output_layer = []
				dec_mux_series = []
				dec_muy_series = []
				dec_sigmax_series = []
				dec_sigmay_series = []
				dec_pi_series = []
				for t_idx in range(len(dec_fc_layer2)):
					dec_output_layer.append(linear(dec_fc_layer2[t_idx], self.output_vec_dim, t_idx))
			...
			if args.output_pred_state_dim > 4:
				...
				for enc_mu, enc_sigma, dec_mux, dec_muy, dec_sigmax, dec_sigmay, prior_mu, prior_sigma, out in \
					...
					sigmax = tf.maximum(tf.exp(tf.split(dec_sigmax, self.prediction_horizon, axis=1)), 0.2)
					sigmay = tf.maximum(tf.exp(tf.split(dec_sigmay, self.prediction_horizon, axis=1)), 0.2)
	...
	def run(self, sess, feed_dict_train):
		_, batch_loss, kl_loss, _current_state, \
		_current_state_lstm_concat, \
		_model_prediction, _summary_str, lr, beta, div_loss = sess.run([self.update,
		                                                                                                  self.total_loss,
		                                                                                                  self.kl_loss,
		                                                                                                  self.current_state,
		...
		return batch_loss, kl_loss, _model_prediction, _summary_str, lr, beta, 0, div_loss, 0
	...
	def predict(self, sess, feed_dict_train, update_state=True):
		_current_state,	_current_state_lstm_concat, \
	...
	def get_log_lossfunc(self, dec_mux, dec_muy, dec_sigmax, dec_sigmay, x, y):
		...
		# Normalization constant
		denom = tf.math.log(2 * np.pi * sxsy)
		# Final PDF calculation
		result = denom + z
		gaussian = tf.reduce_sum(result, axis=1)  # do inner summation
