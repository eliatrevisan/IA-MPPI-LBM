...
class NetworkModel():
  ...
  def __init__(self, args,is_training=True,batch_size = None):
    ...
    self.rnn_state_ped_size = args.rnn_state_ped_size
    ...
    self.n_peds = self.args.n_other_agents
    ...
    if self.args.relative_info:
      self.input_ped_grid_placeholder = tf.placeholder(dtype=tf.float32, shape=[self.batch_size, self.truncated_backprop_length,self.n_peds, self.pedestrian_vector_dim], name='ped_grid')
    if "prev" in self.args.others_info:
      self.input_ped_grid_placeholder = tf.placeholder(dtype=tf.float32,
                                                       shape=[self.batch_size, self.truncated_backprop_length,
                                                              self.n_peds,
                                                              self.pedestrian_vector_dim *(self.prev_horizon+1)],
                                                       name='ped_grid')
    else:
      self.input_ped_grid_placeholder = tf.placeholder(dtype=tf.float32,
                                                       shape=[self.batch_size, self.truncated_backprop_length,
                                                              self.n_peds,
                                                              self.pedestrian_vector_dim * self.prediction_horizon],
                                                       name='ped_grid')
    ...
    self.ped_gru_states_current = np.zeros([args.batch_size,self.n_peds, self.rnn_state_size_lstm_ped])
    self.ped_dec_gru_states_current = np.zeros([args.batch_size,self.n_peds, self.pedestrian_vector_dim * self.prediction_horizon])
    self.fw_ped_states_current = np.zeros([args.batch_size, args.rnn_state_size_lstm_ped])
    self.bw_ped_states_current = np.zeros([args.batch_size, args.rnn_state_size_lstm_ped])
    ...
    self.test_ped_gru_states_current = np.zeros([args.batch_size,self.n_peds, self.rnn_state_size_lstm_ped])
    self.test_ped_dec_gru_states_current = np.zeros([args.batch_size, self.n_peds, self.pedestrian_vector_dim * self.prediction_horizon])
    self.test_fw_ped_states_current = np.zeros([args.batch_size, args.rnn_state_size_lstm_ped])
    self.test_bw_ped_states_current = np.zeros([args.batch_size, args.rnn_state_size_lstm_ped])
    ...
    # Pedestrian LSTM
    self.ped_gru_states = tf.placeholder(dtype=tf.float32, shape=[self.batch_size,self.n_peds, self.rnn_state_size_lstm_ped],
                                              name='cell_state_lstm_ped')
    self.ped_dec_gru_states = tf.placeholder(dtype=tf.float32,
                                         shape=[self.batch_size, self.n_peds, self.pedestrian_vector_dim * self.prediction_horizon],
                                         name='cell_dec_state_lstm_ped')
    self.fw_ped_states = tf.placeholder(dtype=tf.float32, shape=[self.batch_size, self.rnn_state_size_lstm_ped],
                                              name='fw_cell_state_lstm_ped')
    self.bw_ped_states = tf.placeholder(dtype=tf.float32, shape=[self.batch_size, self.rnn_state_size_lstm_ped],
                                        name='bw_cell_state_lstm_ped')
    ...
    with tf.variable_scope("model") as scope:
      with tf.variable_scope('encoder_model') as scope:
        ...
        ped_grid_series = tf.unstack(self.input_ped_grid_placeholder, axis=2)

        # LSTM cells to process pedestrian
        self.cell_outputs_series_lstm_ped = []
        self.current_state_lstm_ped = []
        self.other_peds_states = []
        self.other_peds_dec_states = []
        self.other_peds_gru_outputs = []
        out_ped_concat = []
        with tf.variable_scope('lstm_ped',reuse=tf.AUTO_REUSE) as scope:
          # TODO: try to share weights
          w_enc_state_lstm_ped = self.get_weight_variable(name='w_enc_state_lstm_ped',
                                                          shape=[self.rnn_state_size_lstm_ped,
                                                                 self.rnn_state_ped_size],
                                                          trainable=self.is_training)
          b_enc_state_lstm_ped = self.get_bias_variable(name='b_enc_state_lstm_ped', shape=[1, self.rnn_state_ped_size],
                                                        trainable=self.is_training)

          b_dec_state_lstm_ped = self.get_bias_variable(name='b_dec_state_lstm_ped',
                                                        shape=[1, self.rnn_state_size_lstm_ped],
                                                        trainable=self.is_training)
          for i, ped in enumerate(ped_grid_series):
            # TODO: ADAPT for ped input sequence
            ped_enc_cell = tf.contrib.rnn.GRUCell(self.rnn_state_size_lstm_ped, name='ped_enc_lstm_cell'+str(i), trainable=True)
            ped_dec_cell = tf.contrib.rnn.GRUCell(self.pedestrian_vector_dim * self.prediction_horizon, name='ped_dec_lstm_cell' + str(i),trainable=True)

            ped_input = tf.unstack(ped, axis=1)
            ped_gru_output, ped_gru_state = tf.contrib.rnn.static_rnn(ped_enc_cell,ped_input ,dtype=tf.float32,initial_state=self.ped_gru_states[:,i,:])
            self.other_peds_states.append(ped_gru_state)

            ped_gru_output_fc = tf.add(tf.matmul(ped_gru_output[0], w_enc_state_lstm_ped), b_enc_state_lstm_ped)

            ped_dec_fc = tf.add(tf.matmul(ped_gru_output_fc, tf.transpose(w_enc_state_lstm_ped)), b_dec_state_lstm_ped)

            ped_dec_gru_output, ped_dec_gru_state = tf.contrib.rnn.static_rnn(ped_dec_cell,[ped_dec_fc],dtype=tf.float32,initial_state=self.ped_dec_gru_states[:,i,:])

            self.other_peds_dec_states.append(ped_dec_gru_state)

            ped_loss = tf.reduce_mean(tf.sqrt(tf.square(tf.subtract(ped_dec_gru_output[0], ped_input[0]))),axis=1)

            out_ped_concat.append(ped_gru_output_fc)

          self.ped_enc_dec_loss = tf.reduce_mean(ped_loss)

          self.other_peds_gru_outputs = out_ped_concat

          self.fw_cell_ped = tf.contrib.rnn.GRUCell(self.rnn_state_size_lstm_ped, name='fw_basic_lstm_cell' + str(i),
                                                    trainable=True)
          self.bw_cell_ped = tf.contrib.rnn.GRUCell(self.rnn_state_size_lstm_ped, name='bw_basic_lstm_cell' + str(i),
                                                    trainable=True)

          outputs, states = tf.nn.bidirectional_dynamic_rnn(cell_fw=self.fw_cell_ped,
                                                             cell_bw=self.fw_cell_ped,
                                                             inputs=tf.stack(self.other_peds_gru_outputs,axis=1),
                                                             dtype=tf.float32,
                                                             initial_state_fw=self.fw_ped_states,
                                                             initial_state_bw=self.bw_ped_states)

          self.cell_outputs_series_lstm_ped = tf.unstack(tf.transpose(tf.concat([outputs[0],outputs[1]],axis=2),[1,0,2]),axis=0)
          self.current_state_lstm_ped = states
          self.other_peds_states = tf.stack(self.other_peds_states,axis=1)
          self.other_peds_dec_states = tf.stack(self.other_peds_dec_states, axis=1)

          w_birnn = self.get_weight_variable(name='w_birnn',
                                          shape=[self.rnn_state_size_lstm_ped*2, self.rnn_state_size_lstm_ped],
                                          trainable=self.is_training)
          b_birnn = self.get_bias_variable(name='b_birnn', shape=[1, self.rnn_state_size_lstm_ped],
                                        trainable=self.is_training)

          self.fused = [tf.add(tf.matmul(lstm_out, w_birnn), b_birnn) for lstm_out in
                     self.cell_outputs_series_lstm_ped]
        ...
        concat_lstm = []
        for lstm_out, ped_feature in zip(self.cell_outputs_series_state, self.fused):
      ...
      self.ped_dec_enc_gradients = tf.gradients(self.ped_enc_dec_loss, train_vars)

      # Clip gradients
      self.grads, _ = tf.clip_by_global_norm(self.gradients, self.grads_clip)
      self.ped_dec_enc_gradients_clipped, _ = tf.clip_by_global_norm(self.ped_dec_enc_gradients, self.grads_clip)
    
      # Apply gradients to update model
      self.update = self.optimizer.apply_gradients(zip(self.grads, train_vars), global_step=global_step)
      self.update_ped_dec_enc = self.optimizer.apply_gradients(zip(self.ped_dec_enc_gradients_clipped, train_vars), global_step=global_step)
      #self.update = self.optimizer.minimize(self.total_loss, global_step=global_step)
      # Tensorboard summary
      
      ## Loss
      tf.summary.scalar('Model Loss', self.total_loss)
      tf.summary.scalar('Pedestrian Encode Decoder Loss', self.ped_enc_dec_loss)
  ...
  def feed_dic(self, **kwargs):
    ...
    return {self.input_state_placeholder: batch_x,
            ...
            self.ped_gru_states: self.ped_gru_states_current,
            self.ped_dec_gru_states: self.ped_dec_gru_states_current,
            self.fw_ped_states: self.fw_ped_states_current,
            self.bw_ped_states: self.bw_ped_states_current,
            self.seq_len: [100],
  ...
  def feed_test_dic(self, batch_x,batch_grid, batch_ped_grid,step,noise=0):
    return {self.input_state_placeholder: np.expand_dims(batch_x[:, step, :], axis=0),
            self.input_grid_placeholder: np.expand_dims(batch_grid[:, step, :, :], axis=0),
            self.input_ped_grid_placeholder: np.expand_dims(batch_ped_grid[:, step, :], axis=1),
            self.step: 0,
            ...
            self.ped_gru_states: self.test_ped_gru_states_current,
            self.ped_dec_gru_states: self.test_ped_dec_gru_states_current,
            self.fw_ped_states: self.test_fw_ped_states_current,
            self.bw_ped_states: self.test_bw_ped_states_current,
            self.seq_len: [100],
    ...
    if np.any(sequence_reset):
      for sequence_idx in range(sequence_reset.shape[0]):
        if sequence_reset[sequence_idx] == 1:
          ...
          self.ped_gru_states_current = np.zeros([self.args.batch_size,self.n_peds, self.args.rnn_state_size_lstm_ped])
          self.ped_dec_gru_states_current = np.zeros([self.args.batch_size, self.n_peds, self.pedestrian_vector_dim * self.prediction_horizon])
          self.fw_ped_states_current = np.zeros([self.args.batch_size, self.args.rnn_state_size_lstm_ped])
          self.bw_ped_states_current = np.zeros([self.args.batch_size, self.args.rnn_state_size_lstm_ped])
    ...
    if np.any(sequence_reset):
      for sequence_idx in range(sequence_reset.shape[0]):
        if sequence_reset[sequence_idx] == 1:
          ...
          self.test_ped_gru_states_current = np.zeros([self.args.batch_size,self.n_peds, self.args.rnn_state_size_lstm_ped])
          self.test_ped_dec_gru_states_current = np.zeros([self.args.batch_size, self.n_peds, self.pedestrian_vector_dim * self.prediction_horizon])
          self.test_fw_ped_states_current = np.zeros([self.args.batch_size, self.args.rnn_state_size_lstm_ped])
          self.test_bw_ped_states_current = np.zeros([self.args.batch_size, self.args.rnn_state_size_lstm_ped])
  ...
  def run(self,sess,feed_dict_train):
    _, batch_loss,  _current_state, _current_state_lstm_ped,_other_peds_states, \
    _other_peds_dec_states, _current_state_lstm_concat,\
    _model_prediction, _summary_str, lr, _ped_dec_loss = sess.run([self.update,
                                                          self.total_loss,
                                                          self.current_state,
                                                          self.current_state_lstm_ped,
                                                          self.other_peds_states,
                                                          ...
                                                          self.summary,
                                                          self.learning_rate,
                                                          self.ped_enc_dec_loss],
                                                          feed_dict=feed_dict_train)
    self.cell_state_current, self.hidden_state_current = _current_state
    self.fw_ped_states_current, self.bw_ped_states_current = _current_state_lstm_ped
    self.ped_gru_states_current = _other_peds_states
    self.ped_dec_gru_states_current = _other_peds_dec_states
    self.cell_state_current_lstm_concat, self.hidden_state_current_lstm_concat = _current_state_lstm_concat

    return batch_loss, 0,_model_prediction, _summary_str, lr, 0, 0,0, 0, 0,_ped_dec_loss
  ...
  def run_ped_encoder(self,sess,feed_dict_train):
    _ = sess.run([self.update_ped_dec_enc],
                                                          feed_dict=feed_dict_train)

  def predict(self,sess,feed_dict_train,update_state=True):
    _current_state, _current_state_lstm_ped,_other_peds_states, \
    _other_peds_dec_states ,_current_state_lstm_concat, \
    _model_prediction, output_decoder, \
    _cell_outputs_series_state, _cell_outputs_series_lstm_ped, \
    _cell_outputs_series_lstm_concat = sess.run([self.current_state,
                                              self.current_state_lstm_ped,
                                              self.other_peds_states,
                                              self.other_peds_dec_states,
                                              self.current_state_lstm_concat,
                                              self.prediction,
                                              self.deconv1,
                                              self.cell_outputs_series_state,
                                              self.cell_outputs_series_lstm_ped,
                                              self.cell_outputs_series_lstm_concat
                                              ],
                                              feed_dict=feed_dict_train)
    if update_state:
      ...
      self.test_fw_ped_states_current, self.test_bw_ped_states_current = _current_state_lstm_ped
      self.test_ped_gru_states_current = _other_peds_states
      self.test_ped_dec_gru_states_current = _other_peds_dec_states
      self.test_cell_state_current_lstm_concat, self.test_hidden_state_current_lstm_concat = _current_state_lstm_concat

    return _model_prediction, output_decoder,_cell_outputs_series_state, _cell_outputs_series_lstm_ped, _cell_outputs_series_lstm_concat
