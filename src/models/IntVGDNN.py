...
if sys.version_info[0] < 3:
  ...
  from vrnn_cell3 import VariationalRNNCell as vrnn_cell
  from tf_utils import *
else:
  from src.external.vrnn_cell3 import VariationalRNNCell as vrnn_cell
...
class NetworkModel():
  ...
  def __init__(self, args,is_training=True,batch_size = None):
    ...
    self.ped_gru_states_current = np.zeros([args.batch_size,self.args.n_other_agents, self.args.rnn_state_ped_size])
    ...
    self.test_ped_gru_states_current = np.zeros([args.batch_size,self.args.n_other_agents, self.args.rnn_state_ped_size])
    ...
    # Pedestrian LSTM
    self.ped_gru_states = tf.placeholder(dtype=tf.float32, shape=[self.batch_size,self.args.n_other_agents, self.args.rnn_state_ped_size],
      ...
      with tf.variable_scope('encoder_model') as scope:
        ...
        with tf.variable_scope('lstm_ped', reuse=tf.AUTO_REUSE) as scope:
          ...
          for i, ped in enumerate(ped_grid_series):
            ...
            ped_cell = tf.contrib.rnn.GRUCell(self.args.rnn_state_ped_size, name='ped_basic_lstm_cell' + str(i),
                                              trainable=True)
    ...
    if np.any(sequence_reset):
      for sequence_idx in range(sequence_reset.shape[0]):
        if sequence_reset[sequence_idx] == 1:
          ...
          self.ped_gru_states_current = np.zeros([self.args.batch_size,self.args.n_other_agents, self.args.rnn_state_ped_size])
    ...
    if np.any(sequence_reset):
      for sequence_idx in range(sequence_reset.shape[0]):
        if sequence_reset[sequence_idx] == 1:
          ...
          self.test_ped_gru_states_current = np.zeros([self.args.batch_size,self.args.n_other_agents, self.args.rnn_state_ped_size])
  ...
  def run(self,sess,feed_dict_train):
    ...
    return batch_loss, 0,_model_prediction, _summary_str, lr, 0, 0,0, 0, 0,0
  ...
  def predict(self,sess,feed_dict_train,update_state=True):
    ...
    return _model_prediction, 0, _cell_outputs_series_state, _cell_outputs_series_lstm_ped, _cell_outputs_series_lstm_concat
