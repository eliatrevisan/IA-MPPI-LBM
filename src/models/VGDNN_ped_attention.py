...
if sys.version_info[0] < 3:
  ...
  from storn_cell import VariationalRNNCell as vrnn_cell
...
class NetworkModel():
  ...
  def __init__(self, args,is_training=True,batch_size = None):
    ...
    with tf.variable_scope("model") as scope:
      ...
      with tf.variable_scope('encoder_model') as scope:
        ...
        wq_ped = tf.unstack(
          tf.stack(([self.fc_layer(output, W_wq_ped, b_wq_ped) for output in ped_grid_feature_series])))
        wk_ped = tf.unstack(
          tf.stack(([self.fc_layer(output, W_wk_ped, b_wk_ped) for output in ped_grid_feature_series])))
        wv_ped = tf.unstack(
          tf.stack(([self.fc_layer(output, W_wv_ped, b_wv_ped) for output in ped_grid_feature_series])))

        ped_scaled_attention = []
        for state, key, value in zip(wq_ped, wk_ped, wv_ped):
          ped_scaled_output, attention_weights = scaled_dot_product_attention(tf.expand_dims(state, axis=1),
                                                                              tf.expand_dims(key, axis=1),
                                                                              tf.expand_dims(value, axis=1), mask=None)
          ped_scaled_attention.append(tf.squeeze(ped_scaled_output, axis=1))
        ...
        with tf.variable_scope('lstm_ped') as scope:
          ...
          self.cell_outputs_series_lstm_ped, self.current_state_lstm_ped = tf.contrib.rnn.static_rnn(self.cell_ped, ped_scaled_attention, dtype=tf.float32, initial_state=self.init_state_tuple_lstm_ped)
  ...
  def feed_test_dic(self, batch_x,batch_grid, batch_ped_grid,batch_target,noise=1.0):
    return {self.input_state_placeholder: batch_x,
            self.input_grid_placeholder: batch_grid,
            self.input_ped_grid_placeholder: batch_ped_grid,
            self.step: 0,
            self.output_placeholder: batch_target,
            self.cell_state: np.random.normal(self.test_cell_state_current.copy(),noise),
            self.hidden_state: np.random.normal(self.test_hidden_state_current.copy(),noise),
            self.cell_state_lstm_grid: np.random.normal(self.test_cell_state_current_lstm_grid.copy(),noise),
            self.hidden_state_lstm_grid: np.random.normal(self.test_hidden_state_current_lstm_grid.copy(),noise),
            #self.cell_state: self.cell_state_current,
            #self.hidden_state: self.hidden_state_current,
            ...
            self.cell_state_lstm_concat: np.random.normal(self.test_cell_state_current_lstm_concat.copy(),0),
            self.hidden_state_lstm_concat: np.random.normal(self.test_hidden_state_current_lstm_concat.copy(),0),
            }
