...
class NetworkModel():
  ...
  def __init__(self, args,is_training=True,batch_size = None):
    ...
    with tf.variable_scope("model") as scope:
      ...
      with tf.variable_scope('encoder_model') as scope:
        ...
        W_wq_concat = self.get_weight_variable(name='w_wq_concat',
                                            shape=[self.prior_hidden_size, self.prior_hidden_size],
                                            trainable=True)
        b_wq_concat = self.get_bias_variable(name='b_wq_concat', shape=[1, self.prior_hidden_size], trainable=True)

        W_wk_concat = self.get_weight_variable(name='w_wk_concat',
                                            shape=[self.prior_hidden_size, self.prior_hidden_size],
                                            trainable=True)
        b_wk_concat = self.get_bias_variable(name='b_wk_concat', shape=[1, self.prior_hidden_size], trainable=True)

        W_wv_concat = self.get_weight_variable(name='w_wv_concat',
                                            shape=[self.prior_hidden_size, self.prior_hidden_size],
                                            trainable=True)
        b_wv_concat = self.get_bias_variable(name='b_wv_concat', shape=[1, self.prior_hidden_size], trainable=True)

        wq_concat = tf.unstack(
          tf.stack(([self.fc_layer(output, W_wq_concat, b_wq_concat) for output in concat_lstm])))
        wk_concat = tf.unstack(
          tf.stack(([self.fc_layer(output, W_wk_concat, b_wk_concat) for output in concat_lstm])))
        wv_concat = tf.unstack(
          tf.stack(([self.fc_layer(output, W_wv_concat, b_wv_concat) for output in concat_lstm])))

        concat_scaled_attention = []
        for state, key, value in zip(wq_concat, wk_concat, wv_concat):
          concat_scaled_output, attention_weights = scaled_dot_product_attention(tf.expand_dims(state,axis=1), tf.expand_dims(key,axis=1), tf.expand_dims(value,axis=1), mask=None)
          concat_scaled_attention.append(tf.squeeze(concat_scaled_output,axis=1))
      ...
      with tf.variable_scope('generation_model') as scope:
        ...
        self.cell_outputs_series_lstm_concat, self.current_state_lstm_concat = tf.contrib.rnn.static_rnn(cell_lstm_concat, concat_scaled_attention, dtype=tf.float32, initial_state=self.init_state_tuple_lstm_concat)
  ...
  def feed_test_dic(self, batch_x,batch_grid, batch_ped_grid,batch_target,noise=0):
    return {self.input_state_placeholder: batch_x,
            self.input_grid_placeholder: batch_grid,
            self.input_ped_grid_placeholder: batch_ped_grid,
            self.step: 0,
