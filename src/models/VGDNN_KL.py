...
if sys.version_info[0] < 3:
  ...
  from src.external.storn_cell import VariationalRNNCell as vrnn_cell
...
class NetworkModel():
    ...
    with tf.variable_scope("model") as scope:
      ...
      if args.output_pred_state_dim > 4:
        for enc_mu, enc_sigma, dec_mux, dec_muy, dec_sigmax,dec_sigmay,prior_mu, prior_sigma, dec_pi, out in \
          ...
          sigmax = tf.maximum(tf.minimum(tf.exp(tf.split(dec_sigmax, self.prediction_horizon, axis=1)),0.5),0.001)
          sigmay = tf.maximum(tf.minimum(tf.exp(tf.split(dec_sigmay, self.prediction_horizon, axis=1)),0.5),0.001)
        ...
        for enc_mu, enc_sigma, dec_mux, dec_muy, dec_sigmax, dec_sigmay, prior_mu, prior_sigma, out in \
          ...
          for prediction_step in range(self.prediction_horizon):
            ...
            div_loss_over_horizon.append(diversity_loss*pow(0.9,-prediction_step))
      ...
      self.total_loss = tf.reduce_mean(loss_list, axis=0) + tf.reduce_mean(kl_loss_list, axis=0)# - tf.maximum(self.div_loss,1.0)
      self.kl_loss = tf.reduce_mean(kl_loss_list, axis=0)
      ...
      self.div_gradients = tf.gradients(-0.1*tf.minimum(self.div_loss,2.0), train_vars)

      # Clip gradients
      self.grads, _ = tf.clip_by_global_norm(self.gradients, self.grads_clip)
      self.div_grads, _ = tf.clip_by_global_norm(self.div_gradients, 0.01)
    
      # Apply gradients to update model
      self.update = self.optimizer.apply_gradients(zip(self.grads, train_vars), global_step=global_step)
      self.div_update = self.optimizer.apply_gradients(zip(self.div_grads, train_vars), global_step=global_step)
  ...
  def run(self,sess,feed_dict_train):
    _, _,batch_loss, kl_loss, _current_state, _current_state_lstm_grid, _current_state_lstm_ped, \
  ...
  def get_diversity_loss(self,mux,muy,sigmax,sigmay):
    ...
    div_loss=0
    for mix_id in range(self.args.n_mixtures-1):
      ...
      div_loss += tfd.kl_divergence(gaussian2,gaussian1,allow_nan_stats=False)
