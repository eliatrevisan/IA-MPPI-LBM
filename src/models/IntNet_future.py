...
class NetworkModel():
  ...
  def __init__(self, args,is_training=True,batch_size = None):
    ...
    self.input_state_placeholder = tf.placeholder(dtype=tf.float32, shape=[self.batch_size, self.truncated_backprop_length, self.input_state_dim*self.prediction_horizon], name='input_state')
    ...
    if "prev" in self.args.others_info:
      self.input_ped_grid_placeholder = tf.placeholder(dtype=tf.float32,
                                                       ...
                                                       name='ped_grid')
    elif "prev" in self.args.others_info:
      self.input_ped_grid_placeholder = tf.placeholder(dtype=tf.float32,
                                                       shape=[self.batch_size, self.truncated_backprop_length,
                                                              self.n_peds,
                                                              self.pedestrian_vector_dim *(self.prev_horizon+1)],
  ...
  def feed_dic(self, **kwargs):
    batch_x = kwargs['batch_y_pred']
    batch_x[:,:,:2] = kwargs['batch_vel'][:,:,:2]
