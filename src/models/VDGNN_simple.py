...
class NetworkModel():
			...
			with tf.variable_scope('generation_model') as scope:
				...
				for enc_mu, enc_sigma, dec_mux, dec_muy, dec_sigmax, dec_sigmay, prior_mu, prior_sigma, dec_pi, out in \
					...
					for prediction_step in range(self.prediction_horizon):
						...
						mse_lossfunc = tf.sqrt(tf.add(tf.square(tf.subtract(mux[prediction_step][:,0], x_data)),  # error in x
						                          tf.square(tf.subtract(muy[prediction_step][:,0], y_data))))  # error in y
	...
	def validation_step(self, sess, feed_dict_validation):
		batch_loss, _mse_loss,_current_state, _current_state_lstm_ped, \
	...
	def train_step(self, sess, feed_dict_train):
		_, batch_loss, _mse_loss, _current_state, _current_state_lstm_ped, \
