...
class NetworkModel():
  ...
  def __init__(self, args,is_training=True,batch_size = None):
    ...
    with tf.variable_scope("model") as scope:
      ...
      with tf.variable_scope('encoder_model') as scope:
        ...
        concat_lstm = []
        for lstm_out, ped_feature in zip(self.cell_outputs_series_state, self.cell_outputs_series_lstm_grid):
      ...
      tf.summary.scalar('Grid Encode Loss', self.autoencoder_loss)
      tf.summary.scalar('learning_rate', self.learning_rate)
      tf.summary.image('Pedestrian Grid', self.deconv1)
  ...
  def feed_dic(self, **kwargs):
    ...
    return {self.input_state_placeholder: batch_x,
            self.input_grid_placeholder: kwargs['batch_ped_grid'],
  ...
  def feed_test_dic(self, batch_x,batch_grid, batch_ped_grid,step,noise=0):
    return {self.input_state_placeholder: np.expand_dims(batch_x[:, step, :], axis=0),
            self.input_grid_placeholder: np.expand_dims(batch_ped_grid[:, step, :, :], axis=0),
            self.step: 0,
  ...
  def predict(self,sess,feed_dict_train,update_state=True):
    ...
    _cell_outputs_series_state,_cell_outputs_series_lstm_grid, \
    ...
    return _model_prediction, output_decoder,_cell_outputs_series_state, _cell_outputs_series_lstm_grid, _cell_outputs_series_lstm_concat
