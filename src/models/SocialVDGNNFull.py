...
if sys.version_info[0] < 3:
  ...
  from vrnn_cell2 import VariationalRNNCell as vrnn_cell
  from tf_utils import *
else:
  from src.external.vrnn_cell2 import VariationalRNNCell as vrnn_cell
...
class NetworkModel():
  ...
  def __init__(self, args,is_training=True,batch_size = None):
    ...
    self.ped_gru_states_current = np.zeros([args.batch_size,self.n_peds, self.rnn_state_ped_size])
    ...
    self.test_ped_gru_states_current = np.zeros([args.batch_size,self.n_peds, self.rnn_state_ped_size])
    ...
    # Pedestrian LSTM
    self.ped_gru_states = tf.placeholder(dtype=tf.float32, shape=[self.batch_size,self.n_peds, self.rnn_state_ped_size],
    ...
    with tf.variable_scope("model") as scope:
      ...
      with tf.variable_scope('encoder_model') as scope:
        ...
        with tf.variable_scope('lstm_ped',reuse=tf.AUTO_REUSE) as scope:
          ...
          for i, ped in enumerate(ped_grid_series):
            ...
            ped_cell = tf.contrib.rnn.GRUCell(self.rnn_state_ped_size, name='ped_basic_lstm_cell'+str(i), trainable=True)
            ped_input = tf.unstack(ped, axis=1)
            ped_gru_output, ped_gru_state = tf.contrib.rnn.static_rnn(ped_cell,ped_input ,dtype=tf.float32,initial_state=self.ped_gru_states[:,i,:])
            self.other_peds_states.append(ped_gru_state)

            if i > 0:
              for j, ped_out in enumerate(ped_gru_output):
                out_ped_concat[j] = tf.concat([out_ped_concat[j],ped_gru_output[j]],axis=1)
            else:
              out_ped_concat = ped_gru_output
          ...
          rnn_state_size_lstm_ped = self.get_weight_variable(name='w_birnn',
          ...
          self.fused = [tf.add(tf.matmul(lstm_out, rnn_state_size_lstm_ped), b_birnn) for lstm_out in
                     self.cell_outputs_series_lstm_ped]
        ...
        concat_lstm = []
        for lstm_out, ped_feature, grid_feature in zip(self.cell_outputs_series_state, self.fused, self.cell_outputs_series_lstm_grid):
          concat_lstm.append(tf.concat([lstm_out, ped_feature, grid_feature], axis=1, name="concatenate_lstm_out_with_grid_and_ped"))
      ...
      with tf.variable_scope('generation_model') as scope:
        ...
        W_out = self.get_weight_variable(name='w_out',
                                         shape=[self.fc_hidden_unit_size,
                                                self.output_dim * self.prediction_horizon],
                                         trainable=self.is_training)
        b_out = self.get_bias_variable(name='b_out', shape=[1, self.output_dim * self.prediction_horizon],
                                       trainable=self.is_training)
        ...
        for t_idx in range(len(dec_fc_layer)):
          ...
          dec_pi_series.append(linear(dec_fc_layer[t_idx], self.args.n_mixtures, 100 + t_idx))
        ...
        for enc_mu, enc_sigma, dec_mux, dec_muy, dec_sigmax, dec_sigmay, prior_mu, prior_sigma, dec_pi, out in \
          ...
          mse_prediction_loss_list = []
          mse_prediction_loss_list2 = []
          kl_mode_loss_list = []
          ...
          for prediction_step in range(self.prediction_horizon):
            ...
            mse_lossfunc = tf.sqrt(tf.add(tf.square(tf.subtract(tf.expand_dims(mux[prediction_step][:,0],axis=1), x_data)),
                                          tf.square(tf.subtract(tf.expand_dims(muy[prediction_step][:,0],axis=1), y_data))))
            mse_lossfunc2 = tf.sqrt(tf.add(tf.square(tf.subtract(tf.expand_dims(mux[prediction_step][:, 1],axis=1), x_data)),
                                                         tf.square(tf.subtract(tf.expand_dims(muy[prediction_step][:, 1],axis=1), y_data))))

            # diversity_loss = self.get_diversity_loss(mux[prediction_step], muy[prediction_step],
            #                                         sigmax[prediction_step], sigmay[prediction_step])

            # div_loss_over_horizon.append(diversity_loss)
            mse_prediction_loss_list.append(tf.minimum(mse_lossfunc,mse_lossfunc2))
          ...
          mse_loss_list.append(tf.reduce_mean(mse_prediction_loss_list))
      ...
      self.mse_loss = tf.reduce_mean(mse_loss_list, axis=0)
      ...
      # Tensorboard summary
      self.loss_summary = tf.summary.scalar('loss', self.mse_loss)
      self.log_loss_summary = tf.summary.scalar('log likelihood loss', self.total_loss)
    ...
    if np.any(sequence_reset):
      for sequence_idx in range(sequence_reset.shape[0]):
        if sequence_reset[sequence_idx] == 1:
          ...
          self.ped_gru_states_current = np.zeros([self.args.batch_size,self.n_peds, self.args.rnn_state_ped_size])
    ...
    if np.any(sequence_reset):
      for sequence_idx in range(sequence_reset.shape[0]):
        if sequence_reset[sequence_idx] == 1:
          ...
          self.test_ped_gru_states_current = np.zeros([self.args.batch_size,self.n_peds, self.args.rnn_state_ped_size])
  ...
  def validation_step(self,sess,feed_dict_validation):
    batch_loss, _mse_loss, _current_state, _current_state_lstm_grid, _current_state_lstm_ped, \
    _other_peds_states, _current_state_lstm_concat, summary = sess.run([self.total_loss,
                                                                        self.mse_loss,
                                                                        self.current_state,
  ...
  def train_step(self,sess,feed_dict_train):
    _, batch_loss, _mse_loss, _current_state, _current_state_lstm_grid, _current_state_lstm_ped,_other_peds_states, \
  ...
  def predict(self,sess,feed_dict_train,update_state=True):
    _current_state, _current_state_lstm_grid, _current_state_lstm_ped,_other_peds_states, \
    _current_state_lstm_concat, \
    _model_prediction, output_decoder, \
    _cell_outputs_series_state, _cell_outputs_series_lstm_ped, \
    _cell_outputs_series_lstm_concat, _likelihood = sess.run([self.current_state,
                                              self.current_state_lstm_grid,
                                              self.current_state_lstm_ped,
                                              self.other_peds_states,
                                              self.current_state_lstm_concat,
  ...
  def get_log_lossfunc(self,dec_mux,dec_muy, dec_sigmax,dec_sigmay,x,y,pi):
