...
class NetworkModel():
  ...
  def __init__(self, args,is_training=True,batch_size = None):
    ...
    conv_grid_feature_series = self.process_grid(self.occ_grid_series)
    ...
    with tf.variable_scope("model") as scope:
      ...
      if args.output_pred_state_dim > 4:
        for enc_mu, enc_sigma, dec_mux, dec_muy, dec_sigmax,dec_sigmay,prior_mu, prior_sigma, dec_pi, out in \
          ...
          div_loss_over_truncated_back_prop.append(tf.reduce_sum(div_loss_over_horizon))
      ...
      self.total_loss = tf.reduce_mean(loss_list, axis=0) + tf.reduce_mean(kl_loss_list, axis=0) - self.div_loss
  ...
  def run(self,sess,feed_dict_train):
    ...
    _model_prediction, _summary_str, lr, beta, output_decoder, div_loss = sess.run([self.update,
                                                          self.total_loss,
                                                          #self.kl_loss,
                                                          self.current_state,
    ...
    return batch_loss, 0, _model_prediction, _summary_str, lr, beta, output_decoder, div_loss
  ...
  def get_log_lossfunc(self,dec_mux,dec_muy, dec_sigmax,dec_sigmay,x,y):
    ...
    x = tf.square(tf.div(normx, tf.square(dec_sigmax)))
    y = tf.square(tf.div(normy, tf.square(dec_sigmay)))

    sxsy = tf.multiply(dec_sigmax, dec_sigmay)

    xy = 2 * normx * normy / sxsy

    likelihood_loss = tf.math.log(tf.maximum(2 * np.pi * sxsy, 1e-6)) + x +y - xy
  ...
  def get_mixture_lossfunc(self,dec_mux,dec_muy, dec_sigmax,dec_sigmay,dec_pi,x,y):
    ...
    x = tf.square(tf.div(normx, tf.maximum(tf.square(dec_sigmax),1e-6)))
    y = tf.square(tf.div(normy, tf.maximum(tf.square(dec_sigmay),1e-6)))

    sxsy = tf.maximum(tf.multiply(dec_sigmax, dec_sigmay),1e-6)

    xy = 2 * normx * normy / sxsy

    likelihood_loss = -tf.math.log(tf.maximum(dec_pi, 1e-6)) +tf.math.log(tf.maximum(2 * np.pi*sxsy, 1e-6)) +x +y - xy
    return tf.reduce_sum(likelihood_loss)#
      ...
      if idx == 0:
        with tf.variable_scope('auto_encoder') as scope:
          ...
          fc_grid_dec_biases = self.get_bias_variable(shape=[conv3_flat.get_shape()[1]],
                                                      name="fc_grid_decode_biases",trainable=False)
          ...
          self.deconv1 = tf.contrib.layers.conv2d_transpose(deconv2, 1, [conv1_kernel_size, conv1_kernel_size],
                                                            conv1_stride_length, padding='SAME',trainable=False)
        ...
        conv1 = tf.contrib.layers.conv2d(grid_batch, conv1_number_filters, [conv1_kernel_size, conv1_kernel_size],
                                         conv1_stride_length, trainable=False, reuse=True, scope='auto_encoder/Conv')
        conv2 = tf.contrib.layers.conv2d(conv1, conv2_number_filters, [conv2_kernel_size, conv2_kernel_size],
                                         conv2_stride_length, trainable=False, reuse=True, scope='auto_encoder/Conv_1')
        conv3 = tf.contrib.layers.conv2d(conv2, conv3_number_filters, [conv3_kernel_size, conv3_kernel_size],
                                         conv3_stride_length, padding='VALID', trainable=False, reuse=True,
