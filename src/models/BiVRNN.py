...
class NetworkModel():
	...
	def __init__(self, args, is_training=True, batch_size=None):
		...
		self.cell_state_current_lstm_ped_fw = np.zeros([args.batch_size, args.rnn_state_size_lstm_ped])
		self.hidden_state_current_lstm_ped_fw = np.zeros([args.batch_size, args.rnn_state_size_lstm_ped])
		self.cell_state_current_lstm_ped_bw = np.zeros([args.batch_size, args.rnn_state_size_lstm_ped])
		self.hidden_state_current_lstm_ped_bw = np.zeros([args.batch_size, args.rnn_state_size_lstm_ped])
		...
		self.test_cell_state_current_lstm_ped_fw = np.zeros([args.batch_size, args.rnn_state_size_lstm_ped])
		self.test_hidden_state_current_lstm_ped_fw = np.zeros([args.batch_size, args.rnn_state_size_lstm_ped])
		self.test_cell_state_current_lstm_ped_bw = np.zeros([args.batch_size, args.rnn_state_size_lstm_ped])
		self.test_hidden_state_current_lstm_ped_bw = np.zeros([args.batch_size, args.rnn_state_size_lstm_ped])
		...
		self.test_natch_ped_grid = np.zeros([args.batch_size, 12, self.pedestrian_vector_dim*self.args.n_other_agents])
		...
		# Pedestrian LSTM
		self.cell_state_lstm_ped_fw = tf.placeholder(dtype=tf.float32, shape=[self.batch_size, self.rnn_state_size_lstm_ped],
		                                          name='cell_state_lstm_ped_fw')  # internal state of the cell (before output gate)
		self.hidden_state_lstm_ped_fw = tf.placeholder(dtype=tf.float32, shape=[None, self.rnn_state_size_lstm_ped],
		                                            name='hidden_state_lstm_ped_fw')  # output of the cell (after output gate)
		self.init_state_tuple_lstm_ped_fw = tf.contrib.rnn.LSTMStateTuple(self.cell_state_lstm_ped_fw, self.hidden_state_lstm_ped_fw)
		self.cell_state_lstm_ped_bw = tf.placeholder(dtype=tf.float32, shape=[self.batch_size, self.rnn_state_size_lstm_ped],
		                                          name='cell_state_lstm_ped_bw')  # internal state of the cell (before output gate)
		self.hidden_state_lstm_ped_bw = tf.placeholder(dtype=tf.float32, shape=[None, self.rnn_state_size_lstm_ped],
		                                            name='hidden_state_lstm_ped_bw')  # output of the cell (after output gate)
		self.init_state_tuple_lstm_ped_bw = tf.contrib.rnn.LSTMStateTuple(self.cell_state_lstm_ped_bw, self.hidden_state_lstm_ped_bw)
		...
		with tf.variable_scope("model") as scope:
			...
			with tf.variable_scope('encoder_model') as scope:
				...
				self.W_pedestrian_grid = self.get_weight_variable(name='w_ped_grid',
				                                                  shape=[ped_grid_out_dim*2, ped_grid_out_dim],
				                                                  trainable=True)
				...
				with tf.variable_scope('lstm_ped') as scope:
					self.cell_ped_fw = tf.nn.rnn_cell.LSTMCell(self.rnn_state_size_lstm_ped, name='basic_lstm_cell_fw', trainable=True)
					self.cell_ped_bw = tf.nn.rnn_cell.LSTMCell(self.rnn_state_size_lstm_ped, name='basic_lstm_cell_fw',
					                                           trainable=True)
					ped_grid_feature_series , self.current_state_lstm_ped = tf.nn.bidirectional_dynamic_rnn(cell_fw=self.cell_ped_fw,
					                                                                                        cell_bw=self.cell_ped_bw,
					                                                                                        inputs=tf.stack(ped_grid_series,axis=1),
					                                                                                        dtype=tf.float32,
					                                                                                        sequence_length=self.seq_length,
					                                                                                        initial_state_fw=self.init_state_tuple_lstm_ped_fw,
					                                                                                        initial_state_bw=self.init_state_tuple_lstm_ped_bw)

					# Embedding layer of pedestrian grid
					self.cell_outputs_series_lstm_ped = self.process_pedestrian_grid(tf.unstack(tf.stack(ped_grid_feature_series,axis=2),axis=1))
			...
			self.total_loss = tf.reduce_mean(loss_list, axis=0)+args.diversity_update*self.div_loss + tf.reduce_mean(kl_loss_list, axis=0)*self.beta
	...
	def feed_dic(self, **kwargs):
		...
		return {self.input_state_placeholder: batch_x,
		        ...
		        self.cell_state_lstm_ped_fw: self.cell_state_current_lstm_ped_fw,
		        self.hidden_state_lstm_ped_fw: self.hidden_state_current_lstm_ped_fw,
		        self.cell_state_lstm_ped_bw: self.cell_state_current_lstm_ped_bw,
		        self.hidden_state_lstm_ped_bw: self.hidden_state_current_lstm_ped_bw,
	...
	def feed_test_dic(self, **kwargs):
		...
		for i, other in enumerate(kwargs['other_agents_pos']):
			n_other_agents[i] = len(other)
		self.test_natch_ped_grid = np.roll(self.test_natch_ped_grid,shift=1,axis=1)
		self.test_natch_ped_grid[:,0] = kwargs["batch_ped_grid"][:, step]
		return {self.input_state_placeholder: np.expand_dims(kwargs["batch_vel"][:, step, :], axis=1),
		        self.input_ped_grid_placeholder: self.test_natch_ped_grid,
		        ...
		        self.cell_state_lstm_ped_fw: self.test_cell_state_current_lstm_ped_fw,
		        self.hidden_state_lstm_ped_fw: self.test_hidden_state_current_lstm_ped_fw,
		        self.cell_state_lstm_ped_bw: self.test_cell_state_current_lstm_ped_bw,
		        self.hidden_state_lstm_ped_bw: self.test_hidden_state_current_lstm_ped_bw,
	...
	def feed_val_dic(self, **kwargs):
		...
		return {self.input_state_placeholder: kwargs["batch_vel"],
		        ...
		        self.cell_state_lstm_ped_fw: np.random.normal(self.test_cell_state_current_lstm_ped_fw.copy(), kwargs["ped_noise"]),
		        self.hidden_state_lstm_ped_fw: np.random.normal(self.test_hidden_state_current_lstm_ped_fw.copy(), kwargs["ped_noise"]),
		        self.cell_state_lstm_ped_bw: np.random.normal(self.test_cell_state_current_lstm_ped_bw.copy(),
		                                                   kwargs["ped_noise"]),
		        self.hidden_state_lstm_ped_bw: np.random.normal(self.test_hidden_state_current_lstm_ped_bw.copy(),
		                                                     kwargs["ped_noise"]),
		        self.cell_state_lstm_concat: self.test_cell_state_current_lstm_concat,
		        self.hidden_state_lstm_concat: self.test_hidden_state_current_lstm_concat,
		        }
		...
		if np.any(sequence_reset):
			for sequence_idx in range(sequence_reset.shape[0]):
				if sequence_reset[sequence_idx] == 1:
					...
					self.cell_state_current_lstm_ped_fw[sequence_idx, :] = np.zeros([1, self.rnn_state_size_lstm_ped])
					self.hidden_state_current_lstm_ped_fw[sequence_idx, :] = np.zeros([1, self.rnn_state_size_lstm_ped])
					self.cell_state_current_lstm_ped_bw[sequence_idx, :] = np.zeros([1, self.rnn_state_size_lstm_ped])
					self.hidden_state_current_lstm_ped_bw[sequence_idx, :] = np.zeros([1, self.rnn_state_size_lstm_ped])
		...
		if np.any(sequence_reset):
			for sequence_idx in range(sequence_reset.shape[0]):
				if sequence_reset[sequence_idx] == 1:
					...
					self.test_cell_state_current_lstm_ped_fw[sequence_idx, :] = np.zeros([1, self.rnn_state_size_lstm_ped])
					self.test_hidden_state_current_lstm_ped_fw[sequence_idx, :] = np.zeros([1, self.rnn_state_size_lstm_ped])
					self.test_cell_state_current_lstm_ped_bw[sequence_idx, :] = np.zeros([1, self.rnn_state_size_lstm_ped])
					self.test_hidden_state_current_lstm_ped_bw[sequence_idx, :] = np.zeros([1, self.rnn_state_size_lstm_ped])
	...
	def update_test_hidden_state(self):
		...
		self.test_cell_state_current_lstm_ped_fw = self.cell_state_current_lstm_ped_fw.copy()
		self.test_hidden_state_current_lstm_ped_fw = self.hidden_state_current_lstm_ped_fw.copy()
		self.test_cell_state_current_lstm_ped_bw = self.cell_state_current_lstm_ped_bw.copy()
		self.test_hidden_state_current_lstm_ped_bw = self.hidden_state_current_lstm_ped_bw.copy()
	...
	def validation_step(self, sess, feed_dict_validation,update = True):
		...
		if update:
			...
			self.cell_state_current_lstm_ped_bw, self.hidden_state_current_lstm_ped_fw = _current_state_lstm_ped[0]
			self.cell_state_current_lstm_ped_bw, self.hidden_state_current_lstm_ped_fw = _current_state_lstm_ped[1]
	...
	def train_step(self, sess, feed_dict_train, step=0):
		...
		if step > 2000 and self.args.diversity_update:
			...
			feed_test_dic = self.feed_val_dic(**dict)
