...
with tf.Session(config=config) as sess:
	...
	for step in range(initial_step,args.total_training_steps):
		...
		#batch_x, batch_vel, batch_pos,batch_goal,batch_grid, batch_ped_grid, batch_y,batch_pos_target, other_agents_pos, new_epoch, train_log = data_roboat.getBatch(dpreps)
		batch_x, batch_vel, batch_pos,batch_goal,batch_grid, batch_ped_grid, batch_y,batch_pos_target, other_agents_pos, new_epoch, train_log = data_roboat.getRoboatBatch(dpreps)
		...
		model.reset_cells(data_roboat.sequence_reset)
		...
		if step % print_freq == 0:
			...
			# Get test batch for model validation
			#validation_dict, val_log = data_roboat.getTestBatch(dpreps)
			validation_dict, val_log = data_roboat.getRoboatValidationBatch(dpreps)
			model.reset_test_cells(data_roboat.val_sequence_reset)
			...
			with open(args.model_path + "/tf_log", 'a') as f:
				f.write(str(step) + '\n')
				f.write("TrainLog: " + str(train_log) + '\n')
				f.write("ValidationLog: " + str(val_log) + '\n')
