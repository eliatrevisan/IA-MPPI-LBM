...
class VariationalRNNCell(tf.contrib.rnn.RNNCell):
    ...
    def __call__(self, x, state, scope=None):
        with tf.variable_scope(scope or type(self).__name__):
            ...
            #   Eq 5
            x_in = x[:,:x.shape[1]-self.args.pedestrian_vector_dim]
            ped_info = x[:,x.shape[1]-self.args.pedestrian_vector_dim:x.shape[1]]
            ...
            with tf.variable_scope("phi_x"):
                x_1 = tf.nn.relu(linear(x_in, self.n_x_1))
            ...
            eps = tf.random_normal(tf.shape(enc_mu), prior_mu, prior_sigma, dtype=tf.float32)
            # z = mu + sigma*epsilon
            ...
            output, state2 = self.lstm(tf.concat(axis=1, values=(x_1, z_1,ped_info)), state)
