    ...
    with tf.variable_scope("model") as scope:
      ...
      for pred, out in zip(self.prediction,
        ...
        for prediction_step in range(self.prediction_horizon):
          ...
          error_x = tf.maximum(tf.abs(tf.subtract(pred[:, idx_x], out[:, idx_x])),0.03)
          error_y = tf.maximum(tf.abs(tf.subtract(pred[:, idx_y], out[:, idx_y])),0.03)
          lossfunc = tf.sqrt(tf.add(tf.square(error_x),  # error in x
                                    tf.square(error_y)))  # error in y
