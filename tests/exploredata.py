...
for i in range(len(data_prep.trajectory_set)):
	# Check from which direction trajectory comes
	if data_prep.trajectory_set[i][1].pose_vec[0,:][0] > 10:
		right_start.append(i)
	elif data_prep.trajectory_set[i][1].pose_vec[0,:][0] < -20:
		left_start.append(i)
	else:
		bottom_start.append(i)

print(len(right_start), len(left_start), len(bottom_start))
...
for i in left_start:
	traj = data_prep.trajectory_set[i][1]
	ax[0].plot(traj.pose_vec[:,0], traj.pose_vec[:,1])
	ax[0].set_title("From left canal")

for i in right_start:
	traj = data_prep.trajectory_set[i][1]
	ax[1].plot(traj.pose_vec[:,0], traj.pose_vec[:,1])
	ax[1].set_title("From right canal")

for i in bottom_start:
	traj = data_prep.trajectory_set[i][1]
	ax[2].plot(traj.pose_vec[:,0], traj.pose_vec[:,1])
	ax[2].set_title("From bottom canal")
